        ...
        return (
            <div className="card" key={artist.id}>
                <div className="card-header" role="tab" id={("card-header" + artist.id.toString()).replace(/ /g,'')}>
                    <h5 className="mb-0">
                        <a data-toggle="collapse" aria-expanded={aria_expanded} href={"#tabpanel-" + artist.id.toString()} onClick={(e) => this.handleRouter(artist.id)} aria-controls={"tabpanel-" + artist.id.toString()}>
                        {artist.name}
                        </a>
                    </h5>
                </div>

                <div id={"tabpanel-" + artist.id.toString()} className={(aria_expanded === true) ? 'collapse show' : "collapse"} role="tabpanel" aria-labelledby={("card-header-" + artist.id.toString()).replace(/ /g,'')} data-parent='#accordion-artists'>
                    <div className="card-body">

                        <div className="card">
                            {(artist.imageOfThis) ? <img className="card-img-top" src={artist.imageOfThis} alt={"Image : " + artist.name} /> : null}
                            <div className="card-body">
                                <h4 className="card-title">{artist.name}</h4>
                                <p className="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                                <a href={artist.href} className="btn btn-primary">Link</a>
                            </div>
                            <br />
                            {
                                (this.state.selectedArtist === artist.id || this.props.match.params.art_id === artist.id) ? //without this test all the album accordians will receive the same id (artist id in the path) which generate an error => only the first album accordian can be opened
                        ...
                        (this.state.items) ?
                            <div id="accordion-artists" role="tablist">
                                ...
                                {this.state.items.map((item) => (
                                    this.artistCard(item, this.props.match.params.art_id === item.id)
                                ))}
                            </div>
                        :
                            <div id="spinner" className="text-center">
